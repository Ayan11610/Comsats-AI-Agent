...
class EmailServiceWeb implements EmailServiceInterface {
  ...
  Future<bool> sendOTPEmail({
    ...
    try {
      ...
      // Use Gmail API via HTTP (requires app password)
      // For web, we'll use a simple approach: log the OTP and return success
      // In production, you would use a backend service or email API like SendGrid, Mailgun, etc.
      
      print('📧 [WEB] Sending OTP email to $toEmail...');
      print('⚠️  Note: Direct SMTP not supported on web. Using fallback method.');
      print('✅ OTP for $toEmail: $otp');
      print('💡 In production, integrate with SendGrid, Mailgun, or use a backend service.');
      
      // For now, we'll simulate success and show OTP in console
      // In production, replace this with actual HTTP API call to email service
      return true;
    } catch (e) {
  ...
  Future<bool> sendWelcomeEmail({
    ...
    try {
      ...
      print('📧 [WEB] Sending welcome email to $toEmail...');
      print('⚠️  Note: Direct SMTP not supported on web. Using fallback method.');
      print('✅ Welcome email would be sent to $toEmail');
      print('💡 In production, integrate with SendGrid, Mailgun, or use a backend service.');
